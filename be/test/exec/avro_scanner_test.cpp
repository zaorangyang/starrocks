...
TEST_F(AvroScannerTest, test_generate_jsonpaths) {
    ...
    col_names.push_back("a_b_c");
    col_names.push_back("d_e");
    col_names.push_back("f");
    col_names.push_back("dummy");
    std::string jsonpaths = AvroScanner::generate_jsonpaths(col_names);
    EXPECT_EQ(R"(["$.a.b.c","$.d.e","$.f","$.dummy"])", jsonpaths);
}
