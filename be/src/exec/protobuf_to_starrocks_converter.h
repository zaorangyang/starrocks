...
bool append_pb_convert(Column* column, PbType pbData, bool is_nullable) {
    if (is_nullable) {
        auto nullable = down_cast<NullableColumn*>(column);
        auto data_column = nullable->data_column().get();
        down_cast<FixedLengthColumn<LocalType>*>(data_column)->append(pbData);
...
// string to string
bool string_pb_convert(Column* column, std::string pbData, bool is_nullable, bool strict_mode, const TypeDescriptor* type_desc) {
    int max_size = 0;
    ...
    if (is_nullable) {
        auto nullable = down_cast<NullableColumn*>(column);
        auto data_column = nullable->data_column().get();
        down_cast<BinaryColumn*>(data_column)->append(s);
        nullable->null_column()->append(0);
    } else {
...
template<typename PbType, typename LocalType>
bool decimalv2_pb_convert(Column* column, PbType pbData, bool is_nullable) {
    DecimalV2Value decimalv2Value(pbData, 0);
    if (is_nullable) {
        auto nullable = down_cast<NullableColumn*>(column);
        auto data_column = nullable->data_column().get();
        down_cast<FixedLengthColumn<DecimalV2Value>*>(data_column)->append(decimalv2Value);
        nullable->null_column()->append(0);
    } else {
...
template<typename PbType, typename LocalType>
bool decimalv3_pb_convert(Column* column, PbType pbData, bool is_nullable, bool strict_mode, int decimal_scale) {
    ...
    if (overflow) {
        ...
        nullable->null_column()->append_nulls(1); 
        return true;
    }

    // append data
    auto decimalv3_column = down_cast<DecimalV3Column<LocalType>*>(data_column);
...
template<typename PbType, typename LocalType>
bool decimalv2_float_pb_convert(Column* column, PbType pbData, bool is_nullable) {
...
template<typename PbType, typename LocalType>
bool decimalv2_double_pb_convert(Column* column, PbType pbData, bool is_nullable) {
...
template<typename PbType, typename LocalType>
bool decimalv3_float_double_pb_convert(Column* column, PbType pbData, bool is_nullable, bool strict_mode, int decimal_scale) {
    // handle nullable column
    Column* data_column = column;
    NullableColumn* nullable = nullptr;
    if (is_nullable) {
...
template<typename PbType, typename LocalType>
bool integer_to_integer_pb_convert(Column* column, PbType pbData, bool is_nullable, bool strict_mode) {
    // handle nullable column
    Column* data_column = column;
    NullableColumn* nullable = nullptr;
    if (is_nullable) {
...
template<typename PbType, typename LocalType>
bool float_to_integer_pb_convert(Column* column, PbType pbData, bool is_nullable, bool strict_mode) {
    // handle nullable column
    Column* data_column = column;
    NullableColumn* nullable = nullptr;
    if (is_nullable) {
